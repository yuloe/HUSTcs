...
//依次按照子句、变元顺序选取变元
int selectLitOne(CNF *cnf, variable *literalValue) {
  int indexValue = 0, i;
  clause *clauseSelected = cnf->clauseHead->next;
  literal *litSelecetd;
  while (clauseSelected->satisfied == 0) {
    clauseSelected = clauseSelected->next;
  }
  litSelecetd = clauseSelected->headb;
  indexValue =
      litSelecetd->noLit > 0 ? -litSelecetd->index : litSelecetd->index;
  if (indexValue > 0) {
    literalValue[indexValue - 1].value = 1;
  } else {
    literalValue[-indexValue - 1].value = 0;
...
//选择文字数量最少的子句，当前子句中所有文字都被赋值后选择另外的子句
int selectLitTwo(CNF *cnf, variable *literalValue) {
  static int leastLitNum = 1;
  clause *tempClause;
  literal *lit;
  int indexValue = 0;
  while (indexValue == 0) {
    ...
    while (tempClause) {
      if (tempClause->literalNum == leastLitNum) {
        if (tempClause->satisfied) {
          lit = tempClause->headb;
          indexValue = lit->noLit > 0 ? -lit->index : lit->index;
          break;
        }
      }
...
int DPLL(CNF *cnf, variable *literalValue) {
  ...
  indexValue = selectLitTwo(cnf, literalValue);
